...
const App = () => {
  ...
  const updateBalance = useCallback(
    async (accountAddress, providerInstance) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_BASE_URL}/mint`,
        {
  ...
  const fetchNFTs = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL}/nfts`
      );
  ...
  return (
    <>
      ...
      <div className="bg-gray-50 p-8">
        ...
        {showDetail && (
          <div className="bg-white shadow-lg mt-6 p-4 rounded-lg text-center">
            <p className="font-medium text-gray-700 text-lg whitespace-pre-line">
              {accountDetails}
            </p>
          </div>
        )}
      ...
      <div className="bg-gray-900 p-12 min-h-screen text-white">
        ...
        <form
          onSubmit={onSubmit}
          className="bg-gray-800 shadow-lg mx-auto p-8 rounded-lg max-w-lg"
        >
        ...
        <h2 className="mt-10 mb-6 font-bold text-3xl text-center">
          Minted NFTs
        </h2>
        <div className="gap-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3">
          {currentNFTs.map((nft) => (
            <div
              key={nft._id}
              className="bg-gray-800 shadow-lg hover:shadow-xl p-4 rounded-lg transition-all"
            >
              <img
                src={nft.tokenURI}
                alt="NFT"
              ...
              <p className="mt-3 text-gray-300 text-sm">
                Token Hash: {shortenHash(nft.txHash)}
              </p>
              <p className="text-gray-300 text-sm">
                Minted At: {formatDate(nft.mintedAt)}
              </p>
            </div>
          ))}
        </div>
